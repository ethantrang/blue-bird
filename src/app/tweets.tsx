    ...
    return optimisticTweets.map((tweet) => (
        <div key={tweet.id} className="border border-gray-800 border-t-0 px-4 py-8 flex">
            <div className="h-12 w-12">
                <Image src={tweet.author.avatar_url.replace(/"/g, '')} alt="user avatar" width={48} height={48} className="rounded-full" />
            </div>

            <div className="ml-4">
                <p>
                    <span className="font-bold">{tweet.author.name.replace(/"/g, '')}</span> 
                    <span className="text-sm ml-2 text-gray-4">{tweet.author.username.replace(/"/g, '')}</span>
                </p>
                <p>{tweet.title}</p>
